"""AAPL.X premium seller: available inventory, native XNT proceeds; no sale amount caps."""
import argparse
import fcntl
import json
import sys
...
STATE = ROOT / 'state' / 'aapl-seller-journal.json'
...
def reference_price():
    value = fetch_reference('apple-xstock')['apple-xstock']
    price = Decimal(str(value['usd']))
    updated = Decimal(str(value['last_updated_at']))
    require(price.is_finite() and price>0 and updated.is_finite(), 'Invalid AAPL.X reference')
    require(0 <= Decimal(str(time.time()))-updated <= 300, 'AAPL.X reference stale or future-dated')
...
def locked_cycle(live=False):
    ...
    require(not journal['halted'], 'AAPL.X seller halted; review journal')
    if not reconcile(journal):
        return {'status':'WAITING_FOR_FINALITY'}
    require(not journal['halted'], 'AAPL.X seller halted after failed transaction')
    price, updated=reference_price()
    snap=prepare(snapshot())
    p=plan(snap,price)
    if not p:
        return {'status':'HOLD','reason':'No premium after costs, or no available AAPL.X',
                'wallet_aapl':str(Decimal(snap['balance_in'])/10**8),'reference_usd':str(price)}
    ...
    report={'status':'SIMULATION_PASSED','amount_aapl':str(Decimal(p['amount_raw'])/10**8),
    ...
    if not live:
        atomic_write(ROOT/'state'/'aapl-seller-last-simulation.json',report)
        return report
...
def runtime_status(live,status):
    import os
    try:
        atomic_write(ROOT/'state'/'aapl-seller-runtime.json',{'pid':os.getpid(),
...
def main():
    parser=argparse.ArgumentParser(description='AAPL.X inventory seller; defaults to simulation')
    parser.add_argument('--live',action='store_true')
    parser.add_argument('--watch',action='store_true')
    args=parser.parse_args()
    from . import execution_barrier
    execution_barrier.arm()
    print('AAPL.X: no total or per-trade amount cap; available inventory only; native XNT proceeds; '+('LIVE' if args.live else 'SIMULATION'),flush=True)
    try:
        with process_lock():
            while True:
        ...
        print('AAPL.X seller stopped: '+message+'. Pending reservations remain.',file=sys.stderr,flush=True)
        return 1
