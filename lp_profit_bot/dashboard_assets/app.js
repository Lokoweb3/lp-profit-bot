...
function render(data) {
  ...
  const purchaseWorkers=[workers.buy_stock_bridge,workers.buy_googl_bridge];
  const purchaseRunning=purchaseWorkers.some(worker=>worker?.running===true);
  const purchaseUnknown=purchaseWorkers.some(worker=>worker?.running==null);
  badge('worker-purchase-badge',purchaseRunning?'Running':purchaseUnknown?'Unknown':'Stopped',purchaseRunning?'good':purchaseUnknown?'warn':'');
  $('worker-purchase-detail').textContent=purchaseRunning?'Confirmed purchase or bridge finality tracking is active':purchaseUnknown?'Cannot determine watcher status':'No purchase or bridge watcher running';
  $('stop-all').disabled=stopBusy || !sessionReady || ['googl','spy','spcx','tsla','conversion','bridge','meta','coin','pltr','amd','nvda','aapl','buy_stock_bridge','buy_googl_bridge'].every(key=>workers[key]?.running===false);
  ...
  $('aapl-price').textContent=money(aapl?.price_usd);
  $('aapl-reference').textContent=money(aapl?.reference_price);
  $('aapl-balance').textContent=fmt(aapl?.balance,8);
  $('aapl-gap').textContent=aapl?.gap_percent==null?'—':(Number(aapl.gap_percent)>=0?'+':'')+Number(aapl.gap_percent).toFixed(2)+'%';
  badge('aapl-status',aapl?.fresh?'Fresh data':data.aapl_error?'Unavailable / stale':'Waiting / stale',aapl?.fresh?'good':'warn');
  $('aapl-detail').textContent=data.aapl_error || (aapl?'Pool liquidity '+money(aapl.liquidity_usd)+' · checked '+age(aapl.fetched_at,data.now):'Loading AAPL.X market and wallet data…');
  const spy=data.spy;
  ...
  for(const key of ['spy','spcx','tsla','meta','coin','pltr','amd','nvda','aapl']){
  ...
  const unknownWorkers=['googl','spy','spcx','tsla','conversion','bridge','meta','coin','pltr','amd','nvda','aapl','buy_stock_bridge','buy_googl_bridge'].some(key=>workers[key]?.running==null);
...
$('stock-purchase-form').addEventListener('submit',async event=>{
  ...
  try{
    ...
    if(result.quote_id){
      ...
      const detail=result.action==='new_purchase'?`New purchase approval: spend ${result.spend_usdc} USDC for ${result.asset}. Quoted ${result.quoted_stock} ${result.asset}; protected minimum ${result.minimum_stock}; bridge minimum ${result.bridge_minimum_stock}. Destination: same wallet on X1.`:result.action==='resume_bridge'?`Bridge-only approval: the purchase is finalized. Bridge ${fmt(result.stock_raw/1e8,8)} ${$('stock-purchase-stock').selectedOptions[0].textContent} to X1. No additional purchase will be made.`:`Status check only: ${result.status.replaceAll('_',' ').toLowerCase()}. This action cannot purchase or submit a bridge.`;
      $('stock-purchase-quote-detail').textContent=detail;
      $('stock-purchase-confirm').textContent=result.action==='new_purchase'?`Confirm ${result.spend_usdc} USDC purchase & bridge`:result.action==='resume_bridge'?'Confirm bridge to X1':'Check recorded status';
...
$('stock-purchase-confirm').addEventListener('click',async()=>{
  ...
  purchaseMessage(quote.action==='new_purchase'?'Submitting the confirmed Solana purchase and bridge plan; do not retry until its status is known.':quote.action==='resume_bridge'?'Submitting only the confirmed bridge; no new purchase will be made.':'Checking recorded status without submitting a purchase or bridge.');
  try{
