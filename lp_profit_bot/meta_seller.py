"""META.X premium seller: available inventory, native XNT proceeds; no sale amount caps."""
import argparse
import fcntl
import json
import sys
...
STATE = ROOT / 'state' / 'meta-seller-journal.json'


from .spy_seller import native_lock, NativeSwapBusy


class ReferenceUnavailable(NinjaError):
    """A reference fetch failed before any transaction submission."""


def reference_price():
    try:
        value = fetch_reference('meta-xstock')['meta-xstock']
    except NinjaError as error:
        raise ReferenceUnavailable(str(error)) from None
    price = Decimal(str(value['usd']))
    updated = Decimal(str(value['last_updated_at']))
    require(price.is_finite() and price>0 and updated.is_finite(), 'Invalid META.X reference')
    require(0 <= Decimal(str(time.time()))-updated <= 300, 'META.X reference stale or future-dated')
...
def locked_cycle(live=False):
    ...
    require(not journal['halted'], 'META.X seller halted; review journal')
    if not reconcile(journal):
        return {'status':'WAITING_FOR_FINALITY'}
    require(not journal['halted'], 'META.X seller halted after failed transaction')
    price, updated=reference_price()
    snap=prepare(snapshot())
    p=plan(snap,price)
    if not p:
        return {'status':'HOLD','reason':'No premium after costs, or no available META.X',
                'wallet_meta':str(Decimal(snap['balance_in'])/10**8),'reference_usd':str(price)}
    ...
    report={'status':'SIMULATION_PASSED','amount_meta':str(Decimal(p['amount_raw'])/10**8),
    ...
    if not live:
        atomic_write(ROOT/'state'/'meta-seller-last-simulation.json',report)
        return report
...
def runtime_status(live,status):
    import os
    try:
        atomic_write(ROOT/'state'/'meta-seller-runtime.json',{'pid':os.getpid(),
...
def main():
    parser=argparse.ArgumentParser(description='META.X inventory seller; defaults to simulation')
    ...
    try:
        with process_lock():
            while True:
                ...
                try:
                    ...
                    if not args.watch:
                        raise
                    report={'status':'WAITING_FOR_REFERENCE',
                            'reason':'Reference provider unavailable; retrying in 60 seconds.',
        ...
        print('META.X seller stopped: '+message+'. Pending reservations remain.',file=sys.stderr,flush=True)
        return 1
