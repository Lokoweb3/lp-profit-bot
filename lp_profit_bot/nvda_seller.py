"""NVDA.X premium seller: available inventory, native XNT proceeds; no sale amount caps."""
import argparse
import fcntl
import json
import sys
...
STATE = ROOT / 'state' / 'nvda-seller-journal.json'
...
def reference_price():
    value = fetch_reference('nvidia-xstock')['nvidia-xstock']
    price = Decimal(str(value['usd']))
    updated = Decimal(str(value['last_updated_at']))
    require(price.is_finite() and price>0 and updated.is_finite(), 'Invalid NVDA.X reference')
    require(0 <= Decimal(str(time.time()))-updated <= 300, 'NVDA.X reference stale or future-dated')
...
def locked_cycle(live=False):
    ...
    require(not journal['halted'], 'NVDA.X seller halted; review journal')
    if not reconcile(journal):
        return {'status':'WAITING_FOR_FINALITY'}
    require(not journal['halted'], 'NVDA.X seller halted after failed transaction')
    price, updated=reference_price()
    snap=prepare(snapshot())
    p=plan(snap,price)
    if not p:
        return {'status':'HOLD','reason':'No premium after costs, or no available NVDA.X',
                'wallet_nvda':str(Decimal(snap['balance_in'])/10**8),'reference_usd':str(price)}
    ...
    report={'status':'SIMULATION_PASSED','amount_nvda':str(Decimal(p['amount_raw'])/10**8),
    ...
    if not live:
        atomic_write(ROOT/'state'/'nvda-seller-last-simulation.json',report)
        return report
...
def runtime_status(live,status):
    import os
    try:
        atomic_write(ROOT/'state'/'nvda-seller-runtime.json',{'pid':os.getpid(),
...
def main():
    parser=argparse.ArgumentParser(description='NVDA.X inventory seller; defaults to simulation')
    parser.add_argument('--live',action='store_true')
    parser.add_argument('--watch',action='store_true')
    args=parser.parse_args()
    print('NVDA.X: no total or per-trade amount cap; available inventory only; native XNT proceeds; '+('LIVE' if args.live else 'SIMULATION'),flush=True)
    try:
        with process_lock():
            while True:
        ...
        print('NVDA.X seller stopped: '+message+'. Pending reservations remain.',file=sys.stderr,flush=True)
        return 1
