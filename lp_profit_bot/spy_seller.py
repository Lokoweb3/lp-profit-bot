"""SPY.X premium seller: available inventory, native XNT proceeds; no sale amount caps."""
import argparse
import fcntl
import json
import sys
...
from .comparison import fetch_reference
from .spy_quotes import snapshot, quote
...
STATE = ROOT / 'state' / 'spy-seller-journal.json'


class NativeSwapBusy(SellerError):
    pass


@contextmanager
def native_lock():
    path = ROOT/'state'/'native-swap.lock'
    base.ensure_private_state_dir(path.parent)
    with base.open_state_lock(path) as stream:
        try:
            fcntl.flock(stream, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError:
            raise NativeSwapBusy('Another native swap is in progress') from None
        yield


def reference_price():
    value = fetch_reference('sp500-xstock')['sp500-xstock']
    price = Decimal(str(value['usd']))
    updated = Decimal(str(value['last_updated_at']))
    require(price.is_finite() and price>0 and updated.is_finite(), 'Invalid SPY.X reference')
    require(0 <= Decimal(str(time.time()))-updated <= 300, 'SPY.X reference stale or future-dated')
...
def read_journal(path=None):
    ...
    require(journal["wallet"] == WALLET and journal["cap_raw"] in (None, LEGACY_CAP_RAW),
...
def locked_cycle(live=False):
    ...
    require(not journal['halted'], 'SPY.X seller halted; review journal')
    if not reconcile(journal):
        return {'status':'WAITING_FOR_FINALITY'}
    require(not journal['halted'], 'SPY.X seller halted after failed transaction')
    price, updated=reference_price()
    snap=prepare(snapshot())
    p=plan(snap,price)
    if not p:
        return {'status':'HOLD','reason':'No premium after costs, or no available SPY.X',
                'wallet_spy':str(Decimal(snap['balance_in'])/10**8),'reference_usd':str(price)}
    ...
    report={'status':'SIMULATION_PASSED','amount_spy':str(Decimal(p['amount_raw'])/10**8),
    ...
    if not live:
        atomic_write(ROOT/'state'/'spy-seller-last-simulation.json',report)
        return report
...
@contextmanager
def process_lock():
    path=ROOT/'state'/'spy-seller.lock'
    base.ensure_private_state_dir(path.parent)
    with base.open_state_lock(path) as stream:
        try:
            fcntl.flock(stream,fcntl.LOCK_EX|fcntl.LOCK_NB)
        except BlockingIOError:
            raise SellerError('Another SPY.X seller is running') from None
        yield
...
def runtime_status(live,status):
    import os
    try:
        atomic_write(ROOT/'state'/'spy-seller-runtime.json',{'pid':os.getpid(),
...
def main():
    parser=argparse.ArgumentParser(description='SPY.X inventory seller; defaults to simulation')
    parser.add_argument('--live',action='store_true')
    parser.add_argument('--watch',action='store_true')
    args=parser.parse_args()
    print('SPY.X: no total or per-trade amount cap; available inventory only; native XNT proceeds; '+('LIVE' if args.live else 'SIMULATION'),flush=True)
    try:
        with process_lock():
            while True:
        ...
        print('SPY.X seller stopped: '+message+'. Pending reservations remain.',file=sys.stderr,flush=True)
        return 1
